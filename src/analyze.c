// analyze.c — verify a dumped Z80-soup population for genuine self-replication.
//
// Loads NPROG*32 raw bytes, finds the most common tapes, and for each program
// tests: byte signatures (LDIR/Load-Push families), functional self-copy onto a
// blank partner (forward or reversed), and RECURSIVE replication (does the
// offspring itself replicate?) — the gold standard that byte-signatures can't
// fake. Also reports how many solve a given polynomial task.
#define CHIPS_IMPL
#include "z80.h"
#include "sandbox.h"
#include "disasm.h"
...
static int contains(const uint8_t*t,const uint8_t*p,int l){
    for(int i=0;i+l<=32;i++){int ok=1;for(int j=0;j<l;j++)if(t[i+j]!=p[j]){ok=0;break;}if(ok)return 1;}return 0;}
static int has_ldir(const uint8_t*t){static const uint8_t s[4][2]={{0xED,0xB0},{0xED,0xB8},{0xED,0xA0},{0xED,0xA8}};
    for(int k=0;k<4;k++)if(contains(t,s[k],2))return 1;return 0;}
static int has_loadpush(const uint8_t*t){static const uint8_t s[4][2]={{0x01,0xC5},{0x11,0xD5},{0x21,0xE5},{0xE5,0x2A}};
    for(int k=0;k<4;k++)if(contains(t,s[k],2))return 1;return 0;}

// A non-zero partner so trivial all-NOP (zero) tapes cannot fake replication.
static void fill_partner(uint8_t*p){ for(int i=0;i<32;i++) p[i]=(uint8_t)(0x5A ^ (i*13)); }

// Run [prog | partner], return offspring (partner half) and best copy alignment.
static int run_copy(z80_t*cpu,const uint8_t*prog,const uint8_t*partner,uint8_t*offspring){
    uint8_t m[SB_MEM]; memcpy(m,prog,32); memcpy(m+32,partner,32);
    sb_run(cpu,m,0,SB_BUDGET);
    memcpy(offspring,m+32,32);
    int fwd=0,rev=0; for(int i=0;i<32;i++){if(m[32+i]==prog[i])fwd++; if(m[32+i]==prog[31-i])rev++;}
    return fwd>rev?fwd:rev;
}
// Recursive replication: offspring must itself copy prog into ITS non-zero partner.
static int true_replicator(z80_t*cpu,const uint8_t*prog){
    uint8_t partner[32]; fill_partner(partner);
    uint8_t child[32];
    if(run_copy(cpu,prog,partner,child) < 28) return 0;    // gen1 copy weak
    uint8_t grand[32];
    int m2 = run_copy(cpu,child,partner,grand);            // does child reproduce?
    return m2 >= 28;
}
static int solves(z80_t*cpu,const uint8_t*prog,int task){
    uint8_t m[SB_MEM];
    for(int n=0;n<16;n++){memcpy(m,prog,32);memset(m+32,0,32);
        sb_result r=sb_run(cpu,m,(uint8_t)n,SB_BUDGET); if(r.out_e!=poly_eval(task,n))return 0;}
    return 1;
}
...
int main(int argc,char**argv){
    ...
    z80_t cpu;
    // ---- population-wide functional stats ----
    long ldir=0,lp=0,frepl=0,trepl=0,solve=0;
    for(int i=0;i<NPROG;i++){
        const uint8_t*t=TAPES+(size_t)i*32;
        if(has_ldir(t))ldir++;
        if(has_loadpush(t))lp++;
        uint8_t off[32],partner[32]; fill_partner(partner);
        if(run_copy(&cpu,t,partner,off)>=30)frepl++;
        if(true_replicator(&cpu,t))trepl++;
        if(solves(&cpu,t,task))solve++;
    }
    printf("\n== population functional analysis (n=%d) ==\n",NPROG);
    printf("  LDIR-family signature : %6.2f%%\n",100.0*ldir/NPROG);
    printf("  Load-Push signature   : %6.2f%%\n",100.0*lp/NPROG);
    ...
    for(int i=0;i<NPROG && shown<12;){
        ...
        if(count>=2){
            ...
            uint8_t off[32],partner[32]; fill_partner(partner);
            int cp=run_copy(&cpu,t,partner,off);
            int tr=true_replicator(&cpu,t);
            printf(" [%5d x %5.1f%%] ",count,100.0*count/NPROG);
            for(int b=0;b<32;b++)printf("%02X",t[b]);
            printf("  copy=%2d/32 %s%s%s%s\n",cp,
                tr?"TRUE-REPL ":"", has_ldir(t)?"LDIR ":"", has_loadpush(t)?"LoadPush ":"",
                solves(&cpu,t,task)?"SOLVES":"");
            shown++;
        }
        i=j;
    }
    // count of distinct tapes
    ...
    // ---- example TRUE replicators, disassembled ----
    printf("\n== example evolved replicators (disassembled) ==\n");
    int ex=0;
    for(int i=0;i<NPROG && ex<6;i++){
        const uint8_t*t=TAPES+(size_t)i*32;
        if(!true_replicator(&cpu,t)) continue;
        // skip near-duplicates of already-shown examples
        char dis[256]; disasm_tape(t,20,dis,sizeof dis);
        printf(" #%d ",ex+1); for(int b=0;b<32;b++)printf("%02X",t[b]);
        printf("\n     %s%s\n", dis, solves(&cpu,t,task)?"  [SOLVES]":"");
        ex++;
        i += NPROG/40; // spread samples across the population
    }
